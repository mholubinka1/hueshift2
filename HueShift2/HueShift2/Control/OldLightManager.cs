...
{
    public class OldLightManager : ILightManager
    {
        private ILogger<OldLightManager> logger;
        private IOptionsMonitor<HueShiftOptions> appOptionsDelegate;

        private IHueClientManager clientManager;
        private ILocalHueClient client;

        private IDictionary<string, ControlPair> lights;

        private List<Light> lightsOnNetwork;
        private IDictionary<string, AppLight> lightsInMemory;

        public OldLightManager(ILogger<OldLightManager> logger, IOptionsMonitor<HueShiftOptions> appOptionsDelegate, IHueClientManager clientManager, ILocalHueClient client)
        {
            this.logger = logger;
            this.appOptionsDelegate = appOptionsDelegate;
            this.clientManager = clientManager;
            this.client = client;
            this.lights = new Dictionary<string, ControlPair>();
            this.lightsOnNetwork = new List<Light>();
            this.lightsInMemory = new Dictionary<string, AppLight>();
        }
        ...
        private async Task<List<Light>> DiscoverLights()
        {
            ...
            if (discoveredLights.Count - lightsOnNetwork.Count >= 0)
            {
                ...
                {
                    logger.LogInformation($"Discovery: discovered {discoveredLights.Count - lightsOnNetwork.Count} new lights on the network.");
                }
            }
            else
            {
                logger.LogInformation($"Discovery: {lightsOnNetwork.Count - discoveredLights.Count} lights are no longer connected to the network.");
        ...
        private async Task RefreshLights(DateTime currentTime)
        {
            ...
            foreach (var light in lightsOnNetwork)
            {
                ...
                {
                    var hueShiftLight = new AppLight(light);
                    if (isExcluded)
                    {
        ...
        private async Task Sync(Light light, AppLight expectedLight, DateTime currentTime, TimeSpan duration)
        {
            ...
            logger.LogInformation($"Syncing light | Id: {light.Id} Name: {light.Name} | from: {new AppLightState(light.State).ToString(true)} | to: {expectedLight.State.ToString(true)}");
        ...
        public async Task ExecuteTransition(AppLightState target, LightCommand command, DateTime currentTime, bool resumeControl)
        {
            await RefreshLights(currentTime);
